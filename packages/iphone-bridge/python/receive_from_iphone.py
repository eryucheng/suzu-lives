...
).expanduser() if os.environ.get("SUZU_LIVES_IPHONE_INBOX_DIR", "").strip() else SCRIPT_DIR / "runtime" / "inbox"
...
def deliver_to_suzu(uid: int, prompt: str, image_paths: list[Path]) -> None:
    ...
    print(
        json.dumps(
            {"type": "iphone-feedback", "uid": uid, "prompt": prompt, "attachments": attachments},
            ensure_ascii=False,
        ),
        flush=True,
    )
    ...
    if not acknowledgement:
        raise RuntimeError("Suzu 已断开 iPhone 反馈接收器")
    try:
        result = json.loads(acknowledgement)
    except json.JSONDecodeError as exc:
        raise RuntimeError("Suzu 返回了无效的 iPhone 反馈确认") from exc
    if result.get("type") != "ack" or result.get("uid") != uid or result.get("accepted") is not True:
        raise RuntimeError(str(result.get("message") or "Suzu 未接受这条 iPhone 反馈"))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="读取手机反馈邮件并通过 Suzu 本地接收器送给 Agent")
...
def main() -> int:
    ...
    if not args.event_stream:
        raise RuntimeError("iPhone 反馈监听只能由 Suzu 软件启动")
