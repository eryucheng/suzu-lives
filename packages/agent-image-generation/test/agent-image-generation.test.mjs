...
test("ComfyUI registry files validate, list, select their configured workflow, and bind reference slots", async () => {
  const values = await fixture(); const configDir = path.join(values.agentRoot, "image-generation"); const workflows = path.join(configDir, "workflows"); await fs.mkdir(workflows, { recursive: true });
  ...
  assert.deepEqual((await listComfyWorkflows({ agentRoot: values.agentRoot })).workflows.map((item) => item.id), ["local"]); assert.equal((await validateComfyWorkflows({ agentRoot: values.agentRoot })).status, "valid");
