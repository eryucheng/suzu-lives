...
运行记录和图片仅写入当前 Agent 的 `<dataRoot>/agents/<agent>/image-generation/`，API 使用成功时把用量事件写入同一 Agent 的 `cost-ledger/events.jsonl`。配置、registry 与 `--out` 都受限于该 Agent 数据目录；API Key 始终来自现有软件连接或环境覆盖。
