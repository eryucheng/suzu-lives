...
`--out` 必须位于当前 Agent 的 Suzu Lives 数据目录；`--config` 必须位于软件统一的 `capabilities/image-generation` 配置目录。图片 API 使用“设置 → API”里为“生图”选定的连接，不读取外部配置或凭据；模型与请求协议由该连接决定。成功 JSON 的 `status: "ok"` 才代表图片已保存。
