...
`--out` 与 `--config` 必须位于当前 Agent 的 Suzu Lives 数据目录。图片 API 使用软件保存的阿里百炼连接（或 `DASHSCOPE_API_KEY` 环境覆盖），不读取外部配置或凭据；无参考图使用 `z-image-turbo`，带参考图使用 `wan2.7-image`。成功 JSON 的 `status: "ok"` 才代表图片已保存。
