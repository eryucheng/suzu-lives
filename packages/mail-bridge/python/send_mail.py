...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"找不到邮箱通道配置文件：{path}") from exc
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"邮箱通道配置文件不是有效 JSON：{path}") from exc
    if not isinstance(value, dict):
        raise RuntimeError("邮箱通道配置文件根节点必须是对象")
    return value
...
def required(value: Any, label: str) -> str:
    ...
    if not text:
        raise RuntimeError(f"缺少邮箱发信配置：{label}")
    return text
...
def password(value: dict[str, Any], mail: dict[str, Any]) -> str:
    ...
    return (
        os.environ.get("SUZU_MAIL_PASSWORD", "").strip()
        or os.environ.get("SUZU_EMAIL_PASSWORD", "").strip()
    )
...
def outbound_settings(config: dict[str, Any]) -> tuple[str, int, str, str, str]:
    ...
    sender = str(outbound.get("sender") or mail.get("username") or os.environ.get("SUZU_MAIL_SENDER", "")).strip()
    recipient = str(outbound.get("recipient") or os.environ.get("SUZU_MAIL_RECIPIENT", "")).strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="通过已配置的邮箱通道发送一封请求邮件")
    parser.add_argument("subject")
    parser.add_argument("content")
    parser.add_argument("--config", type=Path, required=True, help="Suzu Lives 统一邮箱通道配置")
...
if __name__ == "__main__":
    try:
        ...
        print(f"MAIL_BRIDGE_ERROR: {error}", file=sys.stderr)
        raise SystemExit(1)
