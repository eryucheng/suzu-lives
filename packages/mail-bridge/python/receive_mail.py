...
DEFAULT_STATE_PATH = SCRIPT_DIR / "mail_state.json"
INBOX_ENV = os.environ.get("SUZU_LIVES_MAIL_INBOX_DIR", "").strip()
DEFAULT_INBOX_PATH = Path(INBOX_ENV).expanduser() if INBOX_ENV else SCRIPT_DIR / "runtime" / "inbox"
...
def get_mail_password(mail_config: dict[str, Any]) -> str:
    ...
    if env_name:
        ...
        if env_value:
            return env_value
    fallback = os.environ.get("SUZU_MAIL_PASSWORD", "").strip()
    if fallback:
        return fallback
    raise RuntimeError("请在邮箱通道配置中填写 passwordEnv，并在本机环境变量中设置邮箱客户端授权码")
...
def image_prompt(image_paths: list[Path], warnings: list[str]) -> str:
    lines: list[str] = []
    if image_paths:
        lines.append(f"邮件返回了 {len(image_paths)} 张图片：")
...
def deliver_to_suzu(uid: int, prompt: str, image_paths: list[Path]) -> None:
    attachments = [
        {
            "kind": "image",
            "path": str(image_path),
    ...
    print(
        json.dumps(
            {"type": "mail-feedback", "uid": uid, "prompt": prompt, "attachments": attachments},
            ensure_ascii=False,
        ),
        flush=True,
    )
    ...
    if not acknowledgement:
        raise RuntimeError("Suzu 已断开邮件反馈接收器")
    try:
        result = json.loads(acknowledgement)
    except json.JSONDecodeError as exc:
        raise RuntimeError("Suzu 返回了无效的邮件反馈确认") from exc
    if result.get("type") != "ack" or result.get("uid") != uid or result.get("accepted") is not True:
        raise RuntimeError(str(result.get("message") or "Suzu 未接受这条邮件反馈"))
...
def process_new_messages(
    ...
    for uid in search_after(connection, last_uid):
        try:
            ...
            deliver_to_suzu(uid, prompt, image_paths)
            print(f"已交给 Suzu UID={uid}")
            last_uid = uid
            save_state(state_path, last_uid)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="读取匹配主题路由的邮件并通过 Suzu 本地接收器送给 Agent")
    ...
    parser.add_argument("--event-stream", action="store_true", help="由 Suzu 软件接收 JSON 反馈事件")
...
def main() -> int:
    ...
    if not args.event_stream:
        raise RuntimeError("邮件反馈监听只能由 Suzu 软件启动")
