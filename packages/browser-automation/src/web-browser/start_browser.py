...
configured_runtime = os.environ.get("SUZU_LIVES_BROWSER_RUNTIME_DIR", "").strip()
RUNTIME_DIR = Path(configured_runtime).expanduser().resolve() if configured_runtime else HERE / "runtime"
...
def matching_chrome_pids():
    if os.name != "nt":
        raise RuntimeError("当前只支持在 Windows 上停止 Agent 专用 Chrome")
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "无法枚举 Chrome 进程：" + (result.stderr or result.stdout).strip()
        )
    ...
    port_marker = f"--remote-debugging-port={DEBUG_PORT}".lower()
    profile_marker = f"--user-data-dir={PROFILE_DIR}".lower()
    matches = []
    for value in values:
        command_line = str(value.get("CommandLine") or "").lower()
        if port_marker in command_line and profile_marker in command_line:
            matches.append(int(value["ProcessId"]))
    return matches
...
def stop_chrome():
    ...
    for pid in pids:
        ...
        if result.returncode != 0:
            failures.append((pid, (result.stderr or result.stdout).strip()))

    if failures:
        details = "; ".join(f"PID {pid}: {message}" for pid, message in failures)
        raise RuntimeError(f"未能停止 Agent 专用 Chrome：{details}")
...
def main():
    ...
    parser.add_argument("--stop", action="store_true", help="停止本脚本启动的专用Chrome")
