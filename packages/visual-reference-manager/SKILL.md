...
只在用户明确要求维护参考资料库时使用；不要把普通聊天附件自动永久保存。资料副本和清单都由 Suzu Lives 写入软件数据目录，Skill 不携带图片、源码、配置或凭据。

每次都必须明确资料归属：`--scope shared` 是用户的共享资料（家、常用物品、公共风格，以及用户明确指定可共享的本人资料）；`--scope contact` 是当前联系人的专属资料。人物脸、服装和私人物品默认放在 `contact`，不得读取或写入其他联系人的专属资料。
...
`suzu-lives visual-reference-manager init --scope contact`

`suzu-lives visual-reference-manager list --scope shared --query "卧室" --limit 10`

`suzu-lives visual-reference-manager show home.bedroom.door-view --scope shared`

`suzu-lives visual-reference-manager validate --scope contact`
...
`suzu-lives visual-reference-manager apply --scope contact --plan '<计划文件>' --dry-run`

只有 dry-run 成功、没有冲突，且用户已确认本批新增、更新或删除后，才执行同一计划的：

`suzu-lives visual-reference-manager apply --scope contact --plan '<计划文件>'`
