...
test("stable manager preserves init, dry-run/apply, list, show, validate, and data-root ownership", async () => {
  ...
  const preview = await call(["apply", "--scope", "contact", "--plan", planPath, "--dry-run"]);
...
test("apply validates a whole plan before writing any partial library state", async () => {
  ...
  await assert.rejects(() => call(["apply", "--scope", "contact", "--plan", planPath]), VisualReferenceManagerError);
  const result = await call(["list", "--scope", "contact"]);
