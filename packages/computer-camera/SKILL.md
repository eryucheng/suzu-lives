...
可选 `--active-seconds <秒>`、`--warmup-seconds <秒>`；也可用 `SUZU_LIVES_DATA_ROOT` 或 `--data-root <软件数据目录>` 指定软件拥有的数据根。返回 `status: "captured"` 或 `"started"` 时才代表后台 worker 已启动；`outputPath` 与 `statusPath` 都在 Suzu Lives 数据目录，不会写入 Claude 项目。
