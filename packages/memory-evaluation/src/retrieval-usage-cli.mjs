...
import { processPendingRetrievalUsageRequests } from "./retrieval-usage-service.mjs";
...
function parseOptions(values) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (!value.startsWith("--")) throw new Error(`memory-usage-analyze 不支持位置参数：${value}`);
    const key = value.slice(2);
    if (!allowed.has(key)) throw new Error(`memory-usage-analyze 不支持选项 --${key}。`);
    const next = values[index + 1];
    if (next === undefined || String(next).startsWith("--")) {
      throw new Error(`memory-usage-analyze 选项 --${key} 缺少值。`);
    }
...
function boundedMaximum(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 500) {
    throw new Error("memory-usage-analyze 的 --max-requests 必须是 1 到 500 的整数。");
  }
  return parsed;
}
...
export function memoryRetrievalUsageAnalysisUsage() {
  return "suzu-lives memory-usage-analyze [--project-root <Agent项目目录>] [--data-root <软件数据目录>] [--max-requests <1-500>]";
}

/** Explicit, bounded single-worker entry. It never starts a scheduler. */
export async function runMemoryRetrievalUsageAnalysisCli({
  ...
  if (!agentId) throw new Error("memory-usage-analyze 无法定位当前 Agent。 ");
  ...
  let result;
  try {
    result = await processPendingRetrievalUsageRequests({
