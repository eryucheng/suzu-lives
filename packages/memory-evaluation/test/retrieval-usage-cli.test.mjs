...
import { runMemoryRetrievalUsageAnalysisCli } from "../src/retrieval-usage-cli.mjs";

function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "suzu-memory-usage-cli-"));
...
test("memory-usage-analyze always uses the current Agent generator", async (context) => {
  const values = fixture();
  context.after(() => fs.rmSync(values.root, { recursive: true, force: true }));
  let agentFactoryCalls = 0;
  let output = "";
  const result = await runMemoryRetrievalUsageAnalysisCli({
...
test("memory-usage-analyze ignores unrelated software API bindings", async (context) => {
  const values = fixture();
  context.after(() => fs.rmSync(values.root, { recursive: true, force: true }));
  let agentFactoryCalls = 0;
  const result = await runMemoryRetrievalUsageAnalysisCli({
...
test("memory-usage-analyze rejects missing databases and unbounded request counts", async (context) => {
  const values = fixture();
  context.after(() => fs.rmSync(values.root, { recursive: true, force: true }));
