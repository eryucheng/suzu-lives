...
<p align="center">
  当前运行时：Suzu Agent Core · 代码 Agent 与游戏控制会在后续版本分阶段接入
</p>
...
> 当前 Agent Core 已接入文字聊天、流式显示、停止、公开历史，以及 Windows 上的直接 PowerShell、文件与既有命令行能力。长期记忆、会话压缩、图片/视频理解和通用网页操作通过各自的联系人能力启用；当前明确尚未接入的是游戏控制和多 Agent 编排。
...
3. 打开 Suzu Lives，选择联系人工作目录并创建联系人，然后从第一段对话开始。

不需要把现有项目搬进 Suzu Lives。新联系人使用 Suzu 自己的工作区和 `SUZU.md`；已有资料不会被自动导入、修改或删除。安装完成后可在 **设置 → 软件更新** 检查更新。
...
- **日常连接**：在明确配置后接入邮箱通道、微信和已接入网站的专用操作。
...
有关软件与 Agent Core 的边界，请参阅[软件边界说明](docs/architecture.md)；外部能力的中立清单与当前安装器状态见[外部能力清单](docs/external-capabilities.md)。
