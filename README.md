...
当前发布版本通过 Claude Code 连接 Agent。长期目标是让联系人、记忆和可选能力成为运行时无关的产品层：支持更多 Agent 时，通过适配层接入它们，而不是再做一套彼此割裂的联系人系统。
...
- 软件运行数据默认保存在 Windows 的 Suzu Lives 本地数据目录；项目中的私人文件不会被复制进软件安装目录。
- 模型、语音、媒体和外部连接等服务按你的配置使用。需要额外服务或设备的能力，会在启用前单独要求配置。
- 已接入的调用来源可按联系人、会话和模型查看 Token 与估算费用，方便知道时间和成本花在了哪里。

有关软件与 Claude 项目之间的边界，请参阅[软件边界说明](docs/architecture.md)；外部能力的中立清单和当前 Claude Code 安装方式见[外部能力清单](docs/external-capabilities.md)。
