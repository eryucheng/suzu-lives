...
function ConversationSessionSettings({ actions, settings }) {
  ...
  return (
    <aside aria-label="当前联系人设置" className={`conversation-session-settings${settings.visible ? "" : " hidden"}`} id="conversationSettings">
      ...
      <section className="conversation-session-settings__section"><header><div><span>CHAT DISPLAY</span><h2>聊天显示</h2></div></header><div className="conversation-session-settings__checks">{settings.preferences.map((preference) => <label key={preference.key}><input checked={preference.checked} onChange={(event) => { void actions.setDisplayPreference(preference.key, event.currentTarget.checked); }} type="checkbox" />{preference.label}</label>)}<label className="conversation-settings__time-display"><span>时间显示</span><select aria-label="时间显示方式" onChange={(event) => { void actions.setTimeDisplay(event.currentTarget.value); }} value={settings.timeDisplay}><option value="bubble">每条气泡内</option><option value="center">画面中心</option></select></label></div></section>
      <section className="conversation-session-settings__section"><header><div><span>CLAUDE CODE</span><h2>运行权限</h2></div></header><label className="conversation-settings__time-display"><span>审批模式</span><select aria-label="审批模式" onChange={(event) => { void actions.setApprovalMode(event.currentTarget.value); }} value={settings.approvalMode}><option value="default">逐条确认</option><option value="acceptEdits">自动编辑（当前）</option><option value="plan">只规划不执行</option><option value="bypassPermissions">完全跳过审批</option></select></label></section>
