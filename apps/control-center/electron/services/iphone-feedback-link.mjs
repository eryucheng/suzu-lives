...
function receiverScriptPath(paths, { packaged = false, resourcesPath = process.resourcesPath } = {}) {
  if (packaged && clean(resourcesPath)) {
    return path.join(resourcesPath, "app.asar.unpacked", "node_modules", "@suzu-lives", "iphone-bridge", "python", "receive_from_iphone.py");
  }
...
function feedbackEvent(value) {
  ...
  if (clean(source.type) !== "iphone-feedback") return null;
...
export function createIphoneFeedbackLinkService({
  ...
  if (!chat?.sendToSession) throw new Error("iPhone 反馈需要会话聊天服务。 ");
  if (typeof settingsProvider !== "function") throw new Error("iPhone 反馈需要当前软件设置。 ");
  ...
  const start = async () => {
    ...
    try {
      ...
      lastError = clean(error?.message) || "iPhone 配置尚未完成设置。";
      report();
    ...
    output.on("line", (line) => {
      ...
      void (async () => {
        try {
          ...
          lastError = clean(error?.message) || "iPhone 反馈无法投递。";
    ...
    next.once?.("error", (error) => {
      ...
      child = null;
      started = false;
      lastError = clean(error?.message) || "iPhone 反馈监听已停止。";
      report();
    });
    next.once?.("close", () => {
      output.close();
