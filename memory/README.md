...
- 项目根目录的 `user.md`：每轮加载的用户核心档案，只保存明确、稳定、当前有效且高频需要的事实；应保持短小；
- 项目根目录的个人备忘录：Agent 主动选择保留的自传式记忆，不属于事实数据库；
- 具体事件、日期、原话和较少使用的偏好仍由 `rag` 保存，需要时再召回。

用户事实改变时直接修改项目根目录 `user.md` 中的旧条目，历史变化留在 RAG。不要把一次饮食、购买、提及或临时状态写成稳定喜好。要让 Claude Code 每轮加载这份档案，在项目级 `CLAUDE.md` 中引用 `@user.md`。
